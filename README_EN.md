...
> [!IMPORTANT]
> **v1.1.0 Major Update: Advanced Gesture Detection API**
> This version introduces **Shake**, **Swing**, and **6 Static Postures (Tilt/Face)** detection. We have also fully optimized sensor noise filtering and time-invariant algorithms. Motion detection with micro:bit is now more precise and stable than ever!
...
| `shake` | `Button` | Detects if the device is **shaking** (rapidly jittering) |
| `swing` | `Button` | Detects if the device has just **swung** (rapid burst of movement) |
| `tiltLeft` | `Button` | Detects if the device is **tilted left** (A-button side towards the floor) |
| `tiltRight` | `Button` | Detects if the device is **tilted right** (B-button side towards the floor) |
| `tiltUp` | `Button` | Detects if the device is **tilted up** (standing vertically with Logo facing the ceiling) |
| `tiltDown` | `Button` | Detects if the device is **tilted down** (standing vertically with Logo facing the floor) |
| `faceUp` | `Button` | Detects if the device is **facing up** (lying flat with LED screen facing the ceiling) |
| `faceDown` | `Button` | Detects if the device is **facing down** (lying flat with LED screen facing the floor) |

---

## Advanced API

Beyond basic value reading, this package provides advanced gesture detection APIs to make motion-based interactions easier.

### 1. Shake and Swing Detection

You can use properties or methods on `MicrobitInputDevice`:

```csharp
var microbit = MicrobitInputDevice.current;
if (microbit == null) return;

// 1. Check if the device is CURRENTLY shaking
if (microbit.IsShake()) 
{
    Debug.Log("Device is shaking!");
}

// 2. Check if a Swing started THIS frame
...
### 2. Static Posture Detection (Tilt / Face)

These APIs are suitable for detecting the orientation of the device:
...
In the `MicrobitInputRuntime` component's Inspector, you can adjust the following parameters to optimize gesture detection:

- **Shake Threshold**:
    *   **Definition**: The threshold for detecting "dynamic energy".
    *   **Description**: The smaller the value, the more sensitive it is (default: 0.15). it detects the displacement energy generated by rapid back-and-forth movement.
- **Swing Threshold**:
    *   **Definition**: The threshold for detecting "instantaneous G-force".
    *   **Description**: The smaller the value, the easier it is to trigger a swing (default: 1.0). It detects the peak acceleration burst, similar to swinging a sword.
- **Tilt Threshold**:
    *   **Definition**: The gravity distribution threshold (recommended: 0.3 ~ 0.6).
    *   **Description**: The smaller the value, the less tilt angle is required to trigger `tilt` or `face` states (default: 0.4).
- **Gesture Duration**:
    *   **Description**: How long the gesture state (like shake or swing) stays `True` after being triggered (seconds). Default is 0.2s.

---

## Version History

*   **v1.0.0**: Initial release. Supports USB Serial (Binary/CSV) and BLE. Basic gesture detection (shake, swing, tilt) included.
...
### Binding Gestures (Shake / Swing / Posture)

In addition to buttons, you can also bind gesture detection (such as shake, swing, or static posture) to Input Actions. This is very convenient for handling "trigger-based events" (e.g., sword swing attack, jump detection).

**Setup Steps:**
1. Add a new Action in your Input Actions Asset (e.g., `ShakeAction`).
2. Set **Action Type** to `Button`.
3. Select the **Binding** path: `<MicrobitInputDevice>/shake`.

In your code, you can use the `performed` event to receive gesture triggers:
...
public void OnShake(InputAction.CallbackContext context)
{
    ...
    {
        Debug.Log("Shake gesture received!");
    }
}
```
...
### v1.1.0 (Current)
- **New Advanced Gesture APIs**:
    - `IsShake()`: Continuous shaking detection.
    - `IsSwing()`: Instantaneous swing detection.
    - `Posture Detection`: Automatic detection of `tiltLeft` / `tiltRight` / `tiltUp` / `tiltDown` / `faceUp` / `faceDown`.
- **Algorithm Optimization**:
    - Implemented Time-Invariant Filter to solve detection failure during data bursts (common in USB CSV mode).
    - Isolated gravity vector from dynamic acceleration for more precise posture and gesture detection.
    - Fixed false-positive triggers caused by first-frame initialization jumps.
- **Documentation Updates**: Detailed descriptions for API paths, physical orientation definitions, and threshold tuning guides.

### v1.0.0
- Basic Unity Input System integration.
- Support for 3-axis acceleration and A/B buttons.
- Support for BLE, USB Binary, and USB CSV transmission modes.
